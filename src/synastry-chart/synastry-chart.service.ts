  ...
  async generatePdf(body: SynastryDto, fileToken?: string | null, tries = 0) {
    try {
      ...
      console.log(
        ...
        `${process.env.BASEAPIURL}synastry-chart`,
      );
      console.log('Request Body:', JSON.stringify(reqBody, null, 2));

      const resSynastry = await axios.post(
        `${process.env.BASEAPIURL}synastry-chart`,
        reqBody,
        {
          headers: {
      ...
      const reqNatalFirst = {
        subject: reqBody.first_subject,
        language: reqBody.language,
        theme: reqBody.theme,
        wheel_only: reqBody.wheel_only,
      };

      const resNatalFirst = await axios.post(
        `${process.env.BASEAPIURL}birth-chart`,
        reqNatalFirst,
        {
          headers: {
            'x-rapidapi-key': process.env.XRapidAPIKey,
      ...
      const reqNatalSecond = {
        subject: reqBody.second_subject,
        language: reqBody.language,
        theme: reqBody.theme,
        wheel_only: reqBody.wheel_only,
      };

      const resNatalSecond = await axios.post(
        `${process.env.BASEAPIURL}birth-chart`,
        reqNatalSecond,
        {
          headers: {
      ...
      let pages: any = [];
      for (let i = 8; i < 12; i++) {
        const page = await this.promptService.generateSummary(
          resSynastry.data,
          body.lang,
          i,
        );
      ...
      const match = await this.promptService.generateSummary(
        resSynastry.data,
        body.lang,
        1,
      );
      ...
      if (!zodiac_signs)
        throw new Error("Couldn't download file zodiac_signs.json from DB");
      ...
      const status = await this.sendMail(body.email, pdf, {
        name1: body.first_subject.name,
        name2: body.second_subject.name,
      });

      if (fileToken)
        await this.stripeTokensService.createTokenFile(fileToken, {
          fileName: `Synastry Chart for ${first_subject.name} and ${second_subject.name}`,
          ...status,
        });
  ...
  async getFile(token: string) {
    try {
      ...
      if (!data)
        return {
          success: false,
          message: 'The file is still being generated.',
        };
